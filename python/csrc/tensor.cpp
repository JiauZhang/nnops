...
void DEFINE_TENSOR_MODULE(nb::module_ & (m)) {
    ...
    auto &pytensor = nb::class_<Tensor>(m, "Tensor")
        ...
        .def("broadcast_to",nb::overload_cast<const TensorShape &>(&Tensor::broadcast_to, nb::const_))
        .def_prop_ro("dtype", &Tensor::dtype)
        .def_prop_ro("device", &Tensor::device_type)
        .def_prop_ro("data_ptr", nb::overload_cast<>(&Tensor::data_ptr, nb::const_))
